...
- Không lồng `if` bên trong `if`/vòng lặp quá 1 cấp — đây là nguồn dễ sinh bug logic ẩn và khó đọc. Khi gặp pattern "for → if điều kiện A → if điều kiện B", gộp điều kiện bằng `&&`/optional chaining rồi `if` một lần (`const found = node.children && recurse(...); if (found) return found;`), hoặc dùng `continue`/guard clause để thoát sớm khỏi vòng lặp thay vì lồng thêm 1 cấp `if` (xem `findContainingArray`/`findParentKey` trong `CategoryList.tsx` làm mẫu).
...
- Trước khi bắt tay vào tính năng mới (không áp dụng cho fix nhỏ đang dở): rà soát nhanh phần code liên quan ở nhánh làm việc hiện tại của **cả `frontend-admin` lẫn `backend-cms`** (2 repo luôn đi cùng nhau — FE Admin gọi trực tiếp CMS BE), tìm lỗi logic còn tồn đọng quanh khu vực tính năng sắp đụng tới (UI hiển thị sai state, gọi API sai contract Swagger hiện tại, validate FE lệch với BE...). Có lỗi thì sửa trước — commit riêng, có thể trên nhánh `fix/...` tách biệt — rồi mới bắt đầu code tính năng mới, không dồn việc dọn nợ này tới lúc tính năng mới xong mới làm. Mục tiêu: code không bug, logic đúng, dễ maintain, không lặp code.
